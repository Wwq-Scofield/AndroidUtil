...
public final class IOUtil
{
  ...
  public static long skip(InputStream is, long skip)
    throws IOException
  {
    long dest = skip;
    while (skip > 0L)
    ...
    while (skip > 0L)
    {
      int read = readFully(is, SKIP_BUF, 0, (int)Math.min(skip, 4096L));
      if (read < 1) {
        break;
      }
      skip -= read;
  ...
  public static int readFully(InputStream is, byte[] buffer)
    throws IOException
  {
    return readFully(is, buffer, 0, buffer.length);
  }
  
  public static int readFully(InputStream is, byte[] buffer, int offset, int len)
    throws IOException
  {
  ...
  public static void closeQuietly(Object c)
  {
    ...
    try
    {
      ...
      Method m = c.getClass().getDeclaredMethod("close", new Class[0]);
